...
data = np.genfromtxt('aero\\flaps0.polar', skip_header=True)
#data = data[:15]
beta = data[:, 0]
alpha = data[:, 2]
cfx = data[:, 13]
...
X = np.linspace(-10, 20, 16)
Y = np.linspace(0, 20, 11)
Z = np.reshape(cfx, [len(X), len(Y)])
plt.plot_surface(X, Y, Z, cmap='plasma')
plt.show()


'''y_new = spo.curve_fit(func, x_values, y_values)[0]
x_new = np.linspace(min(x_values), max(x_values), 1000)
y_new = func(x_new, y_new[0], y_new[1], y_new[2], y_new[3], y_new[4], y_new[5])
...
popt, pcov = spo.curve_fit(_gaussian, xdata, Z.ravel(), p0)
